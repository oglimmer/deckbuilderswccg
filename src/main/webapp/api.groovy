...
import de.oglimmer.bcg.SwccgProperties
...
def getRESTClient(){
  return new RESTClient("http://" + SwccgProperties.INSTANCE.getDbHost() + ":5984/")
}
def getDBName() {
	return "swlcg/"
}
...
if (params.type=='create') {
	try {
		...
		def response = client.put(path: getDBName() + params.email.toLowerCase(), contentType: JSON, requestContentType:  JSON, body: [password2: hashed, deckList_swccg: []])
		safeSession()		
...
if (params.type=='login') {
	try {
		...
		if(success) {
			...
			session.email = params.email.toLowerCase()			
			CrossContextSession.INSTANCE.saveSessionToServletContext(request)	
			def deckNames = httpResponse.data.deckList_swccg
			builder (
...
if (params.type=='save') {	
	...
	if(params.deckId) {
		...
		def deck = response.data.deckList_swccg.find { it.id == params.deckId }
		...
		builder (
			[deckNames: response.data.deckList_swccg, path: (response.data.picPath?response.data.picPath:'tmp')]
		)
	} else {
...
if (params.type=='delete') {
	...
	// update user
	response = client.get(path: getDBName() + session.email, contentType: JSON, requestContentType:  JSON)	
	def deckNames = response.data.deckList_swccg	
	deckNames = deckNames.findAll { it.id != params.deckId }	
	response.data.deckList_swccg = deckNames	
	response = client.put(path: getDBName() + session.email, contentType: JSON, requestContentType:  JSON, body: response.data)		
	// return new created deck
	builder (
