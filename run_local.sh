...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "deckbuilderswccgnet"; then
			verbosePrint "Starting docker network deckbuilderswccgnet on 10.116.179.0/24"
			docker network create -d bridge --subnet 10.116.179.0/24 --gateway 10.116.179.1 "deckbuilderswccgnet"
		else
			verbosePrint "Docker network deckbuilderswccgnet already running"
		fi
	fi
}
...
Url: http://localhost:8080/deckbuilder-swccg
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="f5d56a22a0cb36a82be638cf460d8cd0"
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    echo "...then browse to http://localhost:8080/deckbuilder-swccg"
		  SHELL
		end
	EOF
	vagrant up
...
if [ "$START_COUCHDB" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_COUCHDB" == "docker" ]; then
		if [ ! -f ".couchdbPid" ]; then
			...
			verbosePrint "docker run --rm -d -p 5984:5984  --net=deckbuilderswccgnet --name=couchdb $ADD_HOST_INTERNAL   couchdb:$TYPE_SOURCE_COUCHDB_VERSION"
			dockerContainerIDcouchdb=$(docker run --rm -d -p 5984:5984 \
				--net=deckbuilderswccgnet \
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080  --net=deckbuilderswccgnet --name=tomcat $ADD_HOST_INTERNAL   -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/deckbuilder-swccg"
